...
class GranularParameters extends ChangeNotifier {
  ...
  void setActive(bool value) {
    _isActive = value;
    _engine.setParameter('granular_active', value ? 1.0 : 0.0);
    notifyListeners();
  }
  
  void setGrainRate(double value) {
    _grainRate = value.clamp(0.1, 100.0);
    _engine.setParameter('granular_grain_rate', _grainRate);
    notifyListeners();
  }
  
  void setGrainDuration(double value) {
    _grainDuration = value.clamp(0.001, 1.0);
    _engine.setParameter('granular_grain_duration', _grainDuration);
    notifyListeners();
  }
  
  void setPosition(double value) {
    _position = value.clamp(0.0, 1.0);
    _engine.setParameter('granular_position', _position);
    notifyListeners();
  }
  
  void setPitch(double value) {
    _pitch = value.clamp(0.1, 4.0);
    _engine.setParameter('granular_pitch', _pitch);
    notifyListeners();
  }
  
  void setAmplitude(double value) {
    _amplitude = value.clamp(0.0, 1.0);
    _engine.setParameter('granular_amplitude', _amplitude);
    notifyListeners();
  }
  
  void setPositionVariation(double value) {
    _positionVariation = value.clamp(0.0, 1.0);
    _engine.setParameter('granular_position_var', _positionVariation);
    notifyListeners();
  }
  
  void setPitchVariation(double value) {
    _pitchVariation = value.clamp(0.0, 2.0);
    _engine.setParameter('granular_pitch_var', _pitchVariation);
    notifyListeners();
  }
  
  void setDurationVariation(double value) {
    _durationVariation = value.clamp(0.0, 1.0);
    _engine.setParameter('granular_duration_var', _durationVariation);
    notifyListeners();
  }
  
  void setPan(double value) {
    _pan = value.clamp(-1.0, 1.0);
    _engine.setParameter('granular_pan', _pan);
    notifyListeners();
  }
  
  void setPanVariation(double value) {
    _panVariation = value.clamp(0.0, 1.0);
    _engine.setParameter('granular_pan_var', _panVariation);
    notifyListeners();
  }
  
  void setWindowType(GrainWindowType type) {
    _windowType = type;
    _engine.setParameter('granular_window_type', type.index.toDouble());
    notifyListeners();
  }
