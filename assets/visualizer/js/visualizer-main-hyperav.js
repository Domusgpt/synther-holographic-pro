/**
 * visualizer-main-hyperav.js - v2.0 HyperAV Integration
 * Production-ready HyperAV audio-reactive visualizer integrated for Flutter synthesizer.
 * - Full HyperAV audio reactivity with microphone input support
 * - Enhanced pitch detection and musical note analysis
 * - Advanced audio-visual parameter mapping
 * - Production-ready Flutter integration bridge
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    window.addEventListener('message', (event) => {
        // Handle microphone request from Flutter
        if (event.data?.type === 'requestMicrophone' && event.data.autoRequest) {
            console.log('Flutter requested microphone access');
            if (!audioContext || !analyser) {
                setupAudio().then((success) => {
                    if (success && reactivityIndicator) {
                        reactivityIndicator.textContent = "AUDIO REACTIVE (MIC)";
                        reactivityIndicator.style.borderColor = "#00ff80";
                        reactivityIndicator.style.animation = 'pulseLabel 1.5s infinite';
                    }
        ...
        if (event.data?.type === 'parameterUpdate' && mainVisualizerCore) {
            ...
            console.log(`Received parameter update: ${parameter} = ${value}`);
            
            // Enhanced synthesizer parameter mapping for HyperAV
            const parameterMap = {
                'filterCutoff': 'rotationSpeed',
                'filterResonance': 'patternIntensity',
                'reverbMix': 'glitchIntensity',
                'masterVolume': 'morphFactor',
                'rotationX': 'dimension',
                'rotationY': 'morphFactor',
                'attackTime': 'gridDensity',
                'releaseTime': 'universeModifier',
                'waveformType': 'colorShift',
                'oscillatorVolume': 'lineThickness',
                // Additional enhanced mappings
                'oscillatorFrequency': 'colorShift',
                'grainSize': 'gridDensity',
                'grainDensity': 'patternIntensity',
                'delayTime': 'universeModifier',
                'delayFeedback': 'glitchIntensity',
                'overallEnergy': 'morphFactor',
                'harmonicComplexity': 'dimension'
            };
            
            const visualizerParam = parameterMap[parameter];
            if (visualizerParam && visualParams.hasOwnProperty(visualizerParam)) {
                // Update the base parameter value
                visualParams[visualizerParam] = value;
                
                // Update the visualizer core
                mainVisualizerCore.updateParameters({ [visualizerParam]: value });
                
                // Update the UI slider if it exists
                const slider = document.getElementById(visualizerParam);
                const display = document.getElementById(`${visualizerParam}-value`);
                if (slider && display) {
                    slider.value = value;
                    const step = slider.step;
                    const decimals = step.includes('.') ? step.split('.')[1].length : 0;
